...
import { useTheme } from '../../app/contexts/ThemeContext';
import colors from '../../shared/ui/constants/baseStyles';
import { useCharacter } from '../../features/character-info/model/hooks/useCharacter';
import CharacterInfo from '../../features/character-info/ui/CharacterInfo';
import { CharacterDetails } from '../../features/character-info/model/types';
...
const CharacterInfoBlock: React.FC<CharacterInfoBlockProps> = ({characterId, navigation, isFavorite, onToggleFavorite}) => {
  const { theme } = useTheme();
  const activeColors = colors[theme.mode];
  const {character, loading, getCharacter} = useCharacter();
  ...
  return (
    <View style={[styles.container, { backgroundColor: activeColors.primary }]}>
      <CharacterInfo character={character} navigation={navigation} isFavorite={isFavorite} onToggleFavorite={onToggleFavorite}/>
    </View>
  );
};
